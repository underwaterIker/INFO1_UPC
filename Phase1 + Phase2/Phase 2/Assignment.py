...
def assign_flight(assig, f):
    ...
    if len(assig.flightsAssignedToAircraft) == 0:
        if f.passengers <= assig.Aircraft.maximumCapacity:
            compatible = True
        else:
            compatible = False
    if len(assig.flightsAssignedToAircraft) != 0:
        if assig.flightsAssignedToAircraft[len(assig.flightsAssignedToAircraft)-1].arrivalAirport == f.departureAirport:
            q = f.departureTime - assig.flightsAssignedToAircraft[len(assig.flightsAssignedToAircraft) - 1].arrivalTime
            if q >= 60:
                if f.passengers <= assig.Aircraft.maximumCapacity:
                    compatible = True
        else:
...
def show_assignment(assig):
    ...
    while i < len(assig.flightsAssignedToAircraft):
        ...
        h1 = int(assig.flightsAssignedToAircraft[i].departureTime // 60)
        m1 = int(assig.flightsAssignedToAircraft[i].departureTime % 60)
        h2 = int(assig.flightsAssignedToAircraft[i].arrivalTime // 60)
        m2 = int(assig.flightsAssignedToAircraft[i].arrivalTime % 60)
        if h1 < 10:
            b = 0
            h1 = str(b) + str(h1)
        if h2 < 10:
...
def plot_assignment(assig):
    ...
    while i < len(assig.flightsAssignedToAircraft):
        ...
        h1[d] = assig.flightsAssignedToAircraft[d].departureTime // 60
        h2[d] = assig.flightsAssignedToAircraft[d].arrivalTime // 60
        h11[d] = (assig.flightsAssignedToAircraft[d].departureTime % 60) / 60
        h22[d] = (assig.flightsAssignedToAircraft[d].arrivalTime % 60) / 60
        h111[d] = h1[d] + h11[d]
        h222[d] = h2[d] + h22[d]
...
def plot_assignments(vector_assig):
    ...
    while i < len(vector_assig):
        ...
        while o < len(vector_assig[i].flightsAssignedToAircraft):
            ...
            h1[d] = vector_assig[i].flightsAssignedToAircraft[k].departureTime // 60
            h2[d] = vector_assig[i].flightsAssignedToAircraft[k].arrivalTime // 60
            h11[d] = (vector_assig[i].flightsAssignedToAircraft[k].departureTime % 60) / 60
            h22[d] = (vector_assig[i].flightsAssignedToAircraft[k].arrivalTime % 60) / 60
            h111[d] = h1[d] + h11[d]
            h222[d] = h2[d] + h22[d]
