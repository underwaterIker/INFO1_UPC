...
def plot_flight(f):
    h1 = f.departureTime // 100
    h2 = f.arrivalTime // 100
    h11 = (f.departureTime % 100)/60
    h22 = (f.arrivalTime % 100)/60
    h111 = h1 + h11
    h222 = h2 + h22
...
def plot_flights(vf):
    ...
    while i < len(vf):
        ...
        while l < len(vf):
            ...
            l += 1
        h1 = int(vf[i].departureTime) // 100
        h2 = int(vf[i].arrivalTime) // 100
        h11 = (int(vf[i].departureTime) % 100) / 60
        h22 = (int(vf[i].arrivalTime) % 100) / 60
        h111 = h1 + h11
        h222 = h2 + h22
        plt.title("Flight Duration")
        plt.yticks(V, B)
...
import Airport
# Definition of the function that, given a vector of flights 'vf', creates a new file 'Operations.kml' with the routes of all the flights in the vector
def map_flights(vf, va):
    if len(vf) == 0:
        return False
    else:
        F = open("Operations.kml", "w")
        F.write('<kml xmlns="http://www.opengis.net/kml/2.2">\n')
        F.write("<Document>\n")
        i = 0
        while i < len(vf):
            a = Airport.search_airport_index(va, str(vf[i].departureAirport))
            b = Airport.search_airport_index(va, str(vf[i].arrivalAirport))
            if a == -1 or b == -1:
                ...
                F.write("\t<Placemark>\n")
                F.write("\t\t<name>")
                F.write(vf[i].departureAirport)
                F.write(" to ")
                F.write(vf[i].arrivalAirport)
                F.write("</name>\n")
                F.write("\t\t<LineString>\n")
